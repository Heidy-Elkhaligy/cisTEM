...
void shift_or_rotate_images::DoInteractiveUserInput( ) {
    ...
    if ( shift_images ) {
        x_shift = my_input->GetFloatFromUser("Wanted X-shift", "The wanted X-shift", "0.0");
        y_shift = my_input->GetFloatFromUser("Wanted Y-shift", "The wanted Y-shift", "0.0");
        z_shift = my_input->GetFloatFromUser("Wanted Z-shift", "The wanted Z-shift", "0.0");
    }
    ...
    if ( rotate_images ) {
        rotation_angle = my_input->GetFloatFromUser("Wanted rotation angle", "The wanted rotation angle", "0.0");
    }
...
bool shift_or_rotate_images::DoCalculation( ) {
    ...
    my_extracted_image.Allocate(my_input_stack.ReturnXSize( ), my_input_stack.ReturnYSize( ), true);
    my_extracted_image.SetToConstant(0.0);
    int output_pos = 1;
    if ( extract_range == true ) { // if extract and rotate/shift
        ...
        for ( int counter = first_image_in_range; counter <= last_image_in_range; counter++ ) {
            ...
            if ( shift_images == true or rotate_images == true ) {
                my_extracted_image.Rotate2DInPlace(rotation_angle, FLT_MAX);
        ...
        for ( int counter = 0; counter < my_input_stack.ReturnNumberOfSlices( ); counter++ ) {
            my_extracted_image.ReadSlice(&my_input_stack, counter + 1);
            my_extracted_image.Rotate2DInPlace(rotation_angle, FLT_MAX);
            my_extracted_image.PhaseShift(x_shift, y_shift, z_shift); //Image::PhaseShift(float wanted_x_shift, float wanted_y_shift, float wanted_z_shift)
            my_extracted_image.WriteSlice(&my_output_extracted_images_stack, output_pos);
            output_pos++;
