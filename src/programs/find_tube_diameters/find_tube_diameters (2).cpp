...
// -----------------------------------------------------------------------------
// Robust local maxima finder (SciPy-style, float-safe, prominence-filtered)
// -----------------------------------------------------------------------------
static void local_maxima_1d(
        const std::vector<float>& x,
        std::vector<int>&         midpoints,
        std::vector<int>&         left_edges,
        std::vector<int>&         right_edges,
        float                     min_prominence = 0.05f, //5% of the signal range
        int                       min_distance   = 10) {
    midpoints.clear( );
    left_edges.clear( );
    right_edges.clear( );
    ...
    // Global range for relative prominence
    ...
    while ( i < i_max ) {
        ...
        if ( x[i] > x[i - 1] + EPS ) {
            ...
            // Plateau
            while ( j < i_max && std::fabs(x[j] - x[i]) < EPS )
                ++j;

            // Falling edge confirms a peak
            if ( x[j] < x[i] - EPS ) {
                ...
                // --- Window-based prominence (column-sum friendly) ---
                int   w         = 10; // tune to wall thickness / blur
                float left_min  = x[left - 1];
                float right_min = x[right + 1];
                float base      = x[mid];
                // our peaks are not sudden peaks but more like a ramp
                for ( int k = std::max(0, mid - w); k <= std::min(n - 1, mid + w); ++k )
                    base = std::min(base, x[k]);

                float prominence = x[mid] - base;

                // Relative prominence if min_prominence < 1
                float prom_thresh = min_prominence < 1.0f
                                            ? min_prominence * global_range
                                            : min_prominence;
                ...
                if ( prominence >= prom_thresh ) {
                    ...
                    if ( ! midpoints.empty( ) &&
                         mid - midpoints.back( ) < min_distance ) {

                        // Keep stronger peak
                        int   last = midpoints.back( );
                        float last_prom =
                                x[last] - std::max(x[last - 1], x[last + 1]);

                        if ( prominence > last_prom ) {
                            midpoints.back( )   = mid;
