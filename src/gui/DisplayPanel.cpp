...
void DisplayPanel::OpenFile(wxString wanted_filename, wxString wanted_tab_title, wxArrayLong* wanted_included_image_numbers, bool keep_scale_and_location_if_possible, bool force_local_survey) {
    ...
    // OLD Method: if there is only one image, then set single image mode to true by default
    //Now The single image mode and fialment picking should be available to enable
    if ( is_from_display_program ) {
        if ( my_panel->included_image_numbers.GetCount( ) == 1 ) {
            ...
            my_panel->filament_picking_mode_enabled = true;
        }
        else {
    ...
    // if ( my_panel->filament_picking_mode_enabled ) {
    //     my_panel->image_picking_mode_enabled = false;
    // }
...
//adding a mode selection function to facilitate goind between different modes instead of having the boolean (true/false) option to activate the image picking mode vs coords mode
// adding this function here so that the show_selection_distances is already
void DisplayPanel::SetActiveSelectionMode(SelectionMode mode) {
    // This perfectly synchronizes your existing booleans!
    ReturnCurrentPanel( )->image_picking_mode_enabled    = (mode == MODE_IMAGE);
    ReturnCurrentPanel( )->coords_mode_enabled           = (mode == MODE_COORDS);
    ReturnCurrentPanel( )->filament_picking_mode_enabled = (mode == MODE_FILAMENT);

    // You can also handle your distances boolean here
    ReturnCurrentPanel( )->show_selection_distances = (mode == MODE_FILAMENT);
}
...
void DisplayNotebookPanel::OnRightDown(wxMouseEvent& event) {
    ...
    else if ( show_selection_distances && ! parent_display_panel->popup_exists ) {
...
void DisplayNotebookPanel::OnMotion(wxMouseEvent& event) {
    ...
    if ( event.m_rightDown && show_selection_distances ) {
        ...
        if ( closest_index != -1 && closest_distance < threshold ) {
            long image_number = coord_tracker->coords[closest_index].image_number;

            // Count total points in this image
            int point_count = 0;
            for ( long i = 0; i < coord_tracker->number_of_coords; ++i ) {
            ...
            // Find the last point in this image
            long last_index_in_image = -1;
            for ( long i = coord_tracker->number_of_coords - 1; i >= 0; --i ) {
                if ( coord_tracker->coords[i].image_number == image_number ) {
            ...
            // Try to find a valid pair based on index
            long pair_index = -1;
            if ( closest_index % 2 == 0 ) {
                if ( closest_index + 1 < coord_tracker->number_of_coords &&
                     coord_tracker->coords[closest_index + 1].image_number == image_number ) {
                                    ...
                                    if ( single_image ) {
                                        ...
                                        if ( show_selection_distances && coord_tracker->number_of_coords >= 2 ) {
                                            for ( long i = 1; i < coord_tracker->number_of_coords; i += 2 ) {
                                                if ( coord_tracker->coords[i].image_number == counter &&
                                                     coord_tracker->coords[i - 1].image_number == counter ) {
                                                    int x1 = (coord_tracker->coords[i - 1].x_pos - single_image_x) * actual_scale_factor;
                                                    int y1 = (coord_tracker->coords[i - 1].y_pos - single_image_y) * actual_scale_factor;
                                                    int x2 = (coord_tracker->coords[i].x_pos - single_image_x) * actual_scale_factor;
                                                    int y2 = (coord_tracker->coords[i].y_pos - single_image_y) * actual_scale_factor;
...
void CoordTracker::ToggleCoord(long wanted_image, long wanted_x, long wanted_y) {
    ...
    // if it wasn't found, then check if we can add it first
    if ( ! was_found ) {
        // if the user switched to a new image, check previous image for odd count
        if ( last_selected_image != -1 && wanted_image != last_selected_image ) {
            ...
            if ( count % 2 != 0 ) {
                wxMessageBox(wxString::Format(
                                     "Image %ld has an odd number of selected points (%d).\n"
                                     "Please add or remove a point there before selecting a new image.",
                                     last_selected_image, count),
                             "Unpaired Selection Warning",
                             wxOK | wxICON_WARNING);
                return; // cancel adding the new point
            }
        }

        // All good, now add the new coordinate
        AddCoord(wanted_image, wanted_x, wanted_y);
...
void CoordTracker::AddCoord(long wanted_image, long wanted_x, long wanted_y) {
    ...
    // wanted_y here is not adjusted to a bottom left starting image will keep it here for now but later when saving the values will adjust the value so the extract_filament program work correctly
    //wxPrintf("Added coordinates x is %li and y is %li \n", wanted_x, wanted_y);
...
void CoordTracker::RemoveCoord(long coord_to_remove) {
    ...
    //else
    {
        ...
        buffer_counter = 0; // removed the floating point here to avoid errors
